...
public struct FloatingForm<Content: View>: View {
    @ViewBuilder let content: (@escaping ValidateClosure) -> Content

    @State private var validators: [FloatFieldValidator] = []
...
struct ValidationPreferenceKey: PreferenceKey {
    static var defaultValue: [FloatFieldValidator] = []

    static func reduce(value: inout [FloatFieldValidator], nextValue: () -> [FloatFieldValidator]) {
        value += nextValue()
    }
}
...
extension EnvironmentValues {
    /// View using this environment value must be wrapped in `FloatingForm`
